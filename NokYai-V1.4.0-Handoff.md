...
**Starting Point:** V1.3.1 deployed (commit 57790c2)  
**Repo:** https://github.com/daronhays-git/NokYai_NestCalc_temp_LP  
**Live:** Deployed via Netlify (auto-deploy from main branch)  
**Dev:** http://localhost:5173  
**Goal:** Replace placeholder content, add analytics, fix agent-flagged issues

---

## SECTION 1: Current State (V1.3.1)
...
- Legal components: LegalModal, PrivacyPolicy, TermsOfService, Disclaimer
...
- Contact form: Netlify Forms with AJAX submission
...
- Fix agent-flagged issues (11 unique findings — see baseline report)
- Social sharing preview image (verify OG image renders correctly)
- Blog/content section (future)

### Agent Baseline Issues (from PORT-6 testing)
| # | File | Finding | Agent |
|---|------|---------|-------|
| 1 | Contact.tsx:65 | Empty catch block | Shield |
| 2 | Footer.tsx:2 | Import order | Shield |
| 3 | Footer.tsx:102 | Static inline color | Eagle |
| 4 | LegalModal.tsx:50 | Dual h1 | Shield + Eagle |
| 5 | Services.tsx:32 | Deprecated nok-amber | Eagle |
| 6 | index.html:9,20,30 | Stale title tags | Lighthouse |
| 7 | index.html | Missing JSON-LD schema | Lighthouse |
| 8 | Legal components | Stale legal content | Lighthouse |
| 9 | README.md | Fixed in PORT-8A | Scribe |
| 10 | CLAUDE.md | False positive | Scribe |
| 11 | README + CLAUDE | Port mismatch fixed | Scribe |
...
```
NokYai LP V1.4.0 — continuing from V1.3.1 (commit 57790c2).
...
**Prompt V1.4.0-P1 — Fix agent-flagged quick wins**
```
V1.4.0-P1 — Fix agent-flagged quick wins

Fix these 5 issues found by Shield and Eagle agents:

1. Contact.tsx:65 — add console.error(error) in the catch block 
   before setStatus('error')

2. Footer.tsx:2 — move the asset import (nokYaiLogo) to after 
   all internal component imports (assets must be last per 
   import order convention)

3. Footer.tsx:102 — replace style={{ color: '#D4C9A8' }} with 
   className="text-nok-caption"

4. LegalModal.tsx:50 — change the <h1> to <h2> (Hero already 
   has the page h1)

5. Services.tsx:32 — replace to-nok-amber with to-nok-gold 
   in the gradient class

Verify build passes after all changes.
```

**Prompt V1.4.0-P2 — Fix stale title tags**
```
V1.4.0-P2 — Fix stale title tags

In index.html, update these three tags that still reference 
the old real estate product:

1. Line 9: <title> — change to something like 
   "Custom AI Development Studio | NestCalc.ai"
2. Line 20: og:title — match the new title
3. Line 30: twitter:title — match the new title

Keep the meta description as-is (it's already correct).
Do not change any component files.
```

**Prompt V1.4.0-P3 — Add Organization JSON-LD schema**
```
V1.4.0-P3 — Add Organization JSON-LD schema

Add a JSON-LD structured data block to index.html before 
</head>. Use @type: Organization with:

- name: NestCalc.ai
- url: https://nestcalc.ai
- logo: https://nestcalc.ai/og-image.png
- email: daron@nestcalc.ai
- contactPoint: type ContactPoint, email, contactType "customer service"
- description: match the meta description

Do not change any component files.
```

**Prompt V1.4.0-P4 — Add Google Analytics**
```
V1.4.0-P4 — Add Google Analytics 4
...
**Prompt V1.4.0-P5 — Lighthouse performance audit**
```
V1.4.0-P5 — Lighthouse performance audit
...
**[NEW] claude-code-action@v1 only supports pull_request triggers** — push events fail with "Unsupported event type: push". Use pull_request for Shield/Eagle, push for Lighthouse (different action).

**[NEW] max_turns is not a valid input** — for claude-code-action@v1, use claude_args: "--max-turns 30" instead.

**[NEW] GitHub secrets are per-repo** — porting workflow files doesn't port CLAUDE_CODE_OAUTH_TOKEN. Must be added manually in each repo's Settings → Secrets.

**[NEW] Use claude setup-token in standalone terminal** — cannot run inside Claude Code session. Generates OAuth token for CI use on Pro/Max plans.

**[NEW] Scribe can false-positive** — CLAUDE.md constants.ts finding was wrong; always verify before acting on Scribe output.

**[NEW] Dead code accumulates silently** — 4 section components were unused but still in the repo. Run PORT-C2 style cleanup periodically.

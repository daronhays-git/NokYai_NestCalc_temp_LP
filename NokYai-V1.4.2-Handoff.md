# NokYai LP V1.4.2 — Handoff

**Created:** April 16, 2026  
**Starting Point:** V1.4.1 deployed (commit 20eb707)  
**Repo:** https://github.com/daronhays-git/NokYai_NestCalc_temp_LP  
**Live:** Deployed via Netlify (auto-deploy from main branch)  
**Dev:** http://localhost:5173  
**Goal:** Content replacement pass, legal doc rewrite, GA4 analytics

---

## SECTION 1: Current State (V1.4.1)
...
- Active section components built and rendering:
  - Navbar (fixed, scroll-aware, mobile menu, Nok/Yai gold logo, "Start Your Project" CTA)
  - Hero (full viewport, animated headline, single "Start Your Project" CTA smooth-scrolls to Contact, particle field)
  ...
  - WhyNokYai (stat counters + differentiator cards + founder bio block)
...
- Effects layer:
  - ParticleField (2D canvas, mouse-reactive + touch-reactive on mobile, gold connection lines)
...
- Mobile touch interaction on ParticleField (push, glow, gold lines, trail)
- SEO meta tags: title, description, OG tags, Twitter cards, canonical URL
- Title tags: "Custom AI Development Studio | NestCalc.ai"
- Organization JSON-LD schema in index.html
- Person JSON-LD schema in index.html (EEAT/AEO — Daron R. Hays founder bio)
- Founder bio block in WhyNokYai section (glassmorphism card, animated)
- Eagle favicon (PNG 48x48 + apple-touch-icon 180x180) — SVG clean at 360 bytes
...
- Stale TODO comments removed from 3 hooks (useScrollProgress, useInView, useMousePosition)
- Lighthouse scores (V1.4.0): 99 Performance / 94 Accessibility / 100 Best Practices / 100 SEO
- Professional quality audit (V1.4.1): 16 PASS, 3 MINOR, 0 FIX
...
### What's Pending
- Rewrite legal documents for AI services (currently references old RE product — Supabase auth, Stripe subscriptions, tiered pricing)
...
### Known MINOR Issues (from V1.4.1 quality audit)
...
The immediate priority is content replacement, legal doc rewrite, and GA4 setup.
```
...
```
NokYai LP V1.4.2 — continuing from V1.4.1 (commit 20eb707).
...
**Prompt V1.4.2-P1 — Diagnostic: Legal document content audit**
```
V1.4.2-P1 — Diagnostic: Legal docs audit

Read all three legal components and report the content 
that needs rewriting for an AI development studio:

1. src/components/legal/PrivacyPolicy.tsx — list every 
   line that references Supabase, Stripe, tiered pricing, 
   RE investment, or NestCalc-specific product features

2. src/components/legal/TermsOfService.tsx — list every 
   line referencing subscription tiers, pricing, Stripe 
   billing, or RE-specific terms

3. src/components/legal/Disclaimer.tsx — list every line 
   referencing Perplexity Sonar, property analysis, RE 
   investment tools, or tier-specific features

For each finding: file, line number, current text.
Do NOT change anything — report only.
```

**Prompt V1.4.2-P2 — Add Google Analytics 4**
```
V1.4.2-P2 — Add Google Analytics 4
...
**Prompt V1.4.2-P3 — Re-run Lighthouse audit**
```
V1.4.2-P3 — Lighthouse audit re-run
...
**Prompt V1.4.2-P4 — Validate JSON-LD with Rich Results Test**
```
V1.4.2-P4 — Validate JSON-LD schemas

After deploy, validate both JSON-LD schemas using:
https://search.google.com/test/rich-results

1. Test the deployed URL
2. Report whether Organization schema validates
3. Report whether Person schema validates
...
**[NEW] Touch events on pointer-events:none canvas need window-level listeners** — ParticleField canvas has pointer-events:none, so touch listeners must be on window with bounds checking to only activate inside the canvas area.

**[NEW] preventDefault only inside canvas bounds** — calling preventDefault on all touch events blocks page scrolling. Check touch position is within canvas rect before preventing default.

**[NEW] Single CTA converts better than dual CTA** — Removed "Explore Services" secondary button from Hero. One clear "Start Your Project" → Contact is cleaner and reduces decision fatigue.

**[NEW] Person + Organization JSON-LD as separate script blocks** — don't merge schema types. Keep them as independent script blocks in index.html for clarity and easier maintenance.
...
| 5 | WhyNokYai | #0f2920 deep | Stats + differentiators + founder bio |
